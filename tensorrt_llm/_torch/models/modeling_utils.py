...
from ..modules.fused_moe import MoE, VanillaMoE, MoEPrefetchManager, MoEPrefetchProxy
...
class DecoderModel(nn.Module, metaclass=PPInitCaller):
    ...
    def __moe_prefetch_init__(
        ...
        self.moe_prefetch_proxy_list = [None for _ in range(model_config.pretrained_config.num_hidden_layers)]
        ...
        if model_config.moe_prefetch_config is not None:
            ...
            for layer_idx in self.moe_prefetch_manager.prefetch_layer_indices:
                self.moe_prefetch_proxy_list[layer_idx] = MoEPrefetchProxy(layer_idx, self.moe_prefetch_manager)
