...
class MoEPrefetchManager:
    ...
    """
    def __init__(self, num_hidden_layers: int, moe_layer_freq: int, add_one_to_layer_idx: bool, first_k_dense_replace: int,
        ...
        self.moe_layer_indices = [
            ...
            if i >= first_k_dense_replace and (i + moe_layer_offset) % moe_layer_freq == 0]
        
        # prefetch layer indices (stride-spaced)
        self.prefetch_layer_indices = [
            self.moe_layer_indices[i] 
            for i in range(self.prefetch_stride - 1, len(self.moe_layer_indices), moe_prefetch_stride)]
        
        # prefill needed layers outside decoder layers
        self.initial_prefetch_layers = self.prefetch_layer_indices[:moe_prefetch_depth]
        
        # cuda streams and events for prefetching (per buffer)
        self.prefetch_streams = [torch.cuda.Stream() for _ in range(self.prefetch_depth)]
        ...
        self.next_prefetch_layer = {
            layer_idx: (
                self.prefetch_layer_indices[i + self.prefetch_depth]
                if i + self.prefetch_depth < len(self.prefetch_layer_indices)
                else None
            )
            for i, layer_idx in enumerate(self.prefetch_layer_indices)}
    ...
    def register_weight(self, layer_id: int, weights: List[torch.Tensor]):
        ...
        if self.weight_dtype is None or self.weight_shapes is None:
            ...
            self._allocate_device_weight_buffers(self.weight_shapes["w3_w1_weight"], self.weight_shapes["w2_weight"])
        else:
            assert self.weight_dtype == weights[0].dtype, f"MoE Dtype mismatch on layer {layer_id}: {self.weight_dtype} != {weights[0].dtype}"
            assert weights[0].shape == self.weight_shapes["w3_w1_weight"], f"MoE w3_w1 Weight shapes mismatch on layer {layer_id}: {self.weight_shapes['w3_w1_weight']} != {weights[0].shape}"
            assert weights[1].shape == self.weight_shapes["w2_weight"], f"MoE w2 Weight shapes mismatch on layer {layer_id}: {self.weight_shapes['w2_weight']} != {weights[1].shape}"
            
        self.weights[layer_id] = [weights[0], weights[1]]
        
    def _allocate_device_weight_buffers(self, w3_w1_shape: Tuple[int, int, int], w2_shape: Tuple[int, int, int]):
        ...
        for _ in range(self.prefetch_depth):
            self.device_weight_buffers.append(
                {
                    "w3_w1_weight": torch.empty(w3_w1_shape, dtype=self.weight_dtype, device=torch.device("cuda")),
                    "w2_weight": torch.empty(w2_shape, dtype=self.weight_dtype, device=torch.device("cuda"))
                }
            )
    ...
    def prefetch_weights(self, cur_stream: torch.cuda.Stream):
        for i, layer_idx in enumerate(self.initial_prefetch_layers):
            ...
            with torch.cuda.stream(self.prefetch_streams[i]):
                self.device_weight_buffers[i]["w3_w1_weight"].copy_(self.weights[layer_idx][0].view(self.weight_dtype), non_blocking=True)
                self.device_weight_buffers[i]["w2_weight"].copy_(self.weights[layer_idx][1].view(self.weight_dtype), non_blocking=True)
    ...
    def get_dst_device_buffers(self, layer_idx: int):
        assert layer_idx in self.layer_to_buffer, f"Layer {layer_idx} not found in registered prefteched layers"
        buffer_idx = self.layer_to_buffer[layer_idx]
        return self.device_weight_buffers[buffer_idx]['w3_w1_weight'], self.device_weight_buffers[buffer_idx]['w2_weight']
    ...
    def get_buffer_n_source(self, layer_idx: int):
        ...
        if prefetch_layer_idx is not None:
            ...
            return self.device_weight_buffers[buffer_idx], self.weights[prefetch_layer_idx]
        else:
...
class MoEPrefetchProxy:
    ...
    def __init__(self, layer_id: int, prefetch_manager: MoEPrefetchManager):
        ...
        self.prefetch_stream = self.prefetch_manager.get_prefetched_stream(self.layer_id)
    ...
    def register_weight(self, w3_w1_weight: torch.Tensor, w2_weight: torch.Tensor):
        ...
        self.prefetch_manager.register_weight(self.layer_id, [w3_w1_weight, w2_weight])
        self.w3_w1_dst_buffer, self.w2_dst_buffer = self.prefetch_manager.get_dst_device_buffers(self.layer_id)
        
    def start_next_layer_prefetching(self, cur_stream: torch.cuda.Stream):
        prefetch_dst, prefetch_src = self.prefetch_manager.get_buffer_n_source(self.layer_id)
        if prefetch_dst is not None and prefetch_src is not None:
            self.prefetch_stream.wait_stream(cur_stream)
            with torch.cuda.stream(self.prefetch_stream):
                    prefetch_dst["w3_w1_weight"].copy_(prefetch_src[0].view(self.weight_dtype), non_blocking=True)
                    prefetch_dst["w2_weight"].copy_(prefetch_src[1].view(self.weight_dtype), non_blocking=True)